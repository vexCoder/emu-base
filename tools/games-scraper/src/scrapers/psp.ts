...
export const getDescriptions = async (gc: GameConsole) => {
  ...
  const x = Xray();
  const listingsURL = `https://psxdatacenter.com/psp/ulist.html`;
  ...
  const list: ListItem[] = await x(listingsURL, "body table.sectiontable tr", [
    {
      title: `td:nth-child(3)`,
      serial: `td:nth-child(2)`,
  ...
  await pMap(
    ...
    async (item) => {
      ...
      const url = `${gc.gameDb}?name=${item.title
        .trim()
        .replace(/\s/g, "+")
        .replace(validChar, "")}&platform_id[]=13`;
