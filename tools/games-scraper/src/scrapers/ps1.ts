...
export const getDescriptions = async (gc: GameConsole) => {
  ...
  const x = Xray();
  const listingsURL = `https://psxdatacenter.com/ulist.html`;
  ...
  await pMap(
    ...
    async (item) => {
      ...
      const url = `${gc.gameDb}?name=${item.title
        ...
        .join(" ")
        .replace(/\s/g, "+")
        .replace(validChar, "")}&platform_id[]=10`;

      console.log(
        `${chalk.bgCyan(
          chalk.black(`[${counter}/${languageMappedList.length}]`)
      ...
      if (!first) {
        console.error(chalk.red(`Failed to find game: ${item.title} (${url})`));

        const id = createHash("SHA256").update(unique).digest("base64");
